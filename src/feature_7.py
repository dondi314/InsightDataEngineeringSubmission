...
class newtimewindow(object):
    def __init__(self, name):
        self.name = name
        self.bandwidth = 0
    def add_to_bandwidth(self, input):
        x, bits = fn.code_bits(input)
        self.bandwidth += bits


def busiest_15_minute_bandwidth(log_txt, busiest_quarter_hours_txt):
    time_window = {}
    for i in range(0,24):
        for j in range(0,4):
            for k in range(0,7):
                time_window[(i,j,k)] = newtimewindow((i,j,k))
        ...
        for line in infile:
            try:
                ...
                if the_minute < 15:
                    ...
                    the_quarter = 3
                time_window[(the_hour, the_quarter, the_day_of_week)].add_to_bandwidth(line)
            except IndexError:
                print "We are skipping this line (it might be blank)"
                print line
    my_count_list = []
    for key in time_window:
        my_count_list.append([time_window[key].bandwidth,key])
    my_count_list.sort()
