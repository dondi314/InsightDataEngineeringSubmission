...
function compare_outputs {
  ...
  PROJECT_ANSWER_PATH5=${GRADER_ROOT}/temp/log_output/top_ten_users_bandwidth.txt
  PROJECT_ANSWER_PATH6=${GRADER_ROOT}/temp/log_output/busiest_quarter_hours.txt
  PROJECT_ANSWER_PATH7=${GRADER_ROOT}/temp/log_output/busiest_15_minute_bandwidth.txt
  ...
  TEST_ANSWER_PATH5=${GRADER_ROOT}/tests/${test_folder}/log_output/top_ten_users_bandwidth.txt
  TEST_ANSWER_PATH6=${GRADER_ROOT}/tests/${test_folder}/log_output/busiest_quarter_hours.txt
  TEST_ANSWER_PATH7=${GRADER_ROOT}/tests/${test_folder}/log_output/busiest_15_minute_bandwidth.txt
  ...
  if [ "${DIFF_RESULT5}" -eq "0" ] && [ -f ${PROJECT_ANSWER_PATH5} ]; then
    echo -e "[${color_green}PASS${color_norm}]: ${test_folder} (top_ten_users_bandwidth.txt)"
    PASS_CNT=$(($PASS_CNT+1))
  else
    echo -e "[${color_red}FAIL${color_norm}]: ${test_folder} (top_ten_users_bandwidth.txt)"
  ...
  if [ "${DIFF_RESULT6}" -eq "0" ] && [ -f ${PROJECT_ANSWER_PATH6} ]; then
    echo -e "[${color_green}PASS${color_norm}]: ${test_folder} (busiest_quarter_hours.txt)"
    PASS_CNT=$(($PASS_CNT+1))
  else
    echo -e "[${color_red}FAIL${color_norm}]: ${test_folder} (busiest_quarter_hours.txt)"
    diff ${PROJECT_ANSWER_PATH6} ${TEST_ANSWER_PATH6}
  fi

  DIFF_RESULT7=$(diff -bB ${PROJECT_ANSWER_PATH7} ${TEST_ANSWER_PATH7} | wc -l)
  if [ "${DIFF_RESULT7}" -eq "0" ] && [ -f ${PROJECT_ANSWER_PATH7} ]; then
    echo -e "[${color_green}PASS${color_norm}]: ${test_folder} (busiest_15_minute_bandwidth.txt)"
    PASS_CNT=$(($PASS_CNT+1))
  else
    echo -e "[${color_red}FAIL${color_norm}]: ${test_folder} (busiest_15_minute_bandwidth.txt)"
    diff ${PROJECT_ANSWER_PATH7} ${TEST_ANSWER_PATH7}
  fi
}

function run_all_tests {
  TEST_FOLDERS=$(ls ${GRADER_ROOT}/tests)
  NUM_TESTS=$(($(echo $(echo ${TEST_FOLDERS} | wc -w)) * 7))
  PASS_CNT=0
